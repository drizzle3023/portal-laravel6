    ...
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill font-size-h2 font-w400 mt-2 mb-0 mb-sm-2">@lang('messages.Edit Product')</h1>
                    ...
                    <div class="row push">
                        ...
                        <div class="col-lg-8 col-xl-5">
                            <div class="form-group">
                                ...
                                <select class="js-select2 form-control" name="customer-id" style="width: 100%;" data-placeholder="Choose one.." disabled>
                                    @foreach($customer_array as $customer)
                                        <option value="{{$customer->id}}" @if($customer->id == $product->customer_id) selected @endif>
                                            {{$customer->email}}
                                        </option>
                                    @endforeach
                                </select>
                                <input type="hidden" name="customer-id" value="{{$product->customer_id}}">
                            </div>
                            <div class="form-group">
                                <label for="dm-project-new-name">
                                ...
                                <select class="js-select2 form-control" name="domain-id" style="width: 100%;" data-placeholder="Choose one..">
                                    @foreach($domain_array as $domain)
                                        <option value="{{$domain->id}}" @if($domain->id == $product->domain_id) selected @endif>
                                            {{$domain->domain}}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="product-name" placeholder="@lang('messages.Product name')" value="{{$product->name}}">
                            </div>
                            <div class="form-group">
                                <label for="dm-project-new-name">
                                    @lang('messages.Allowed users') <span class="text-danger">*</span>
                                </label>
                                <input type="text" class="form-control" name="allowed-users" placeholder="@lang('messages.Allowed users')" value="{{$product->alloweduser}}">
                            </div>
